...
const { consultarCupom } = require('../_onboardingSync');

/** Consulta informativa de disponibilidade de código de cupom — nunca a
 * garantia final (isso é sempre a reserva, no banco do Worker). */
module.exports = async function handler(req, res) {
  ...
  const resultado = await consultarCupom(sessionToken, code);
  if (!resultado.ok) {
    return res.status(resultado.status || 502).json({ error: resultado.error, code: resultado.code });
  }
  return res.status(200).json({ ok: true, normalizedCode: resultado.data.normalizedCode, available: resultado.data.available });
};
